...
#define ADCDIRECTMAX (10) // 'L431 ADC: Number of ADCs read in one scan with DMA
...
// LC = Local (sram) Copy of parameters
 struct ADCLC
 {
	...
	struct ADCCALABS cabsbms[ADCBMSMAX]; // BMS cells: Absolute
	struct ADCCALABS cabsaux[ADCAUXMAX]; // BMS auxillary: Absolute
	struct ADCCALABS cabsadc[ADCDIRECTMAX]; // Processor ADC Absolute readings
 };
